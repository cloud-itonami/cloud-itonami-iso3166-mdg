(ns marketentry.governor-contract-test
  ...
  This is the MDG actor's SIX-check governor (see `marketentry.governor`
  docstring for why it is six, not a richer sibling's seven): there is
  no distinct tax-ID/NIF-establishing-article test here because there is
  no such check -- the dossier does not ground a distinct NIF-
  establishing-article separate from ordinary ARMP exclusion screening
  (Article 21(c)). `armp-ineligibility` is the MDG analog of MOZ's
  `cadastro-unico-missing`/GIN's `pecuniary-sanction-outside-range`
  flagship check, but is itself a genuine TWO-condition test (currently
  listed OR an unlawfully-long prior exclusion on record absent a
  récidive finding), reflecting Article 90 §IX-X's own time-bounded,
  récidive-gated design."
...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "Antananarivo Fabril SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest armp-ineligibility-currently-listed-is-held-and-unoverridable
  (testing "currently on ARMP's own quarterly ineligibility list -> HARD hold (MDG flagship check, listed branch)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:armp-ineligibility} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest armp-ineligibility-unlawful-duration-is-held-and-unoverridable
  (testing "not currently listed, but a 7-year prior exclusion with no récidive finding exceeds Article 90 §IX's 5-year cap -> HARD hold (MDG flagship check, duration branch)"
    (let [[db actor] (fresh)
...
(deftest engagement-not-requiring-armp-clearance-still-commits-end-to-end
  (testing "eng-5 does NOT itself require ARMP clearance (pure private-investment engagement) -- the conditional branch of the flagship check still commits clean"
    (let [[db actor] (fresh)
