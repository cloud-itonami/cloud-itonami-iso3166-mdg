...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co" :jurisdiction "MDG"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "MDG" :checklist ["a"] :spec-basis "x"}})
...
(deftest seed-stores-have-the-demo-engagement-set
  (testing "MemStore and DatomicStore seed to the same 5-engagement demo set"
    (let [mem (store/seed-db)
          dat (store/datomic-seed-db)]
      (is (= 5 (count (store/all-engagements mem))))
      (is (= 5 (count (store/all-engagements dat))))
      (is (= (mapv :id (store/all-engagements mem))
             (mapv :id (store/all-engagements dat)))))))
