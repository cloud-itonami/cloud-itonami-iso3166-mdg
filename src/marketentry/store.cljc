(ns marketentry.store
  "SSoT for the MDG market-entry compliance actor, behind a `Store`
  ...
  MDG's Article 90 ARMP-ineligibility mechanism (Loi n°2016-055, §IX-X)
  is a TIME-BOUNDED exclusion with a récidive escape hatch, not a plain
  two-registry boolean design (this fleet's -moz sibling) nor a
  percentage-of-value pecuniary-sanction range (this fleet's -gin
  sibling) -- so the engagement schema here has FOUR ARMP-related
  fields: `:requires-armp-clearance?` (conditional applicability --
  false for a pure private-investment engagement that never goes
  through public procurement), `:on-armp-ineligibility-list?` (current
  ground truth), `:armp-recidivism-finding?` and
  `:armp-prior-exclusion-duration-years` (together grounding the
  Article 90 §IX 5-year statutory-cap/récidive-escape-hatch test),
  reflecting the actual primary-source mechanism rather than forcing
  MDG's real design into a borrowed shape. See `marketentry.governor`
  for the corresponding flagship-check docstring."
...
(defn demo-data
  ...
  the 5 engagements exercises a distinct governor path:
    ...
    eng-2 -- armp-ineligibility (currently on ARMP's own quarterly
             Liste des personnes physiques et morales inéligibles)
    eng-3 -- armp-ineligibility, the OTHER branch (NOT currently listed,
             but a recorded PRIOR exclusion of 7 years with no récidive
             finding -- unlawfully long under Article 90 §IX's 5-year
             cap, so this actor does not trust the 'no longer listed'
             narrative and holds anyway)
    eng-4 -- engagement-fee-mismatch (claimed-fee != base + months x
             rate)
    eng-5 -- clean, does not itself require ARMP clearance (a pure
             private-investment engagement routed through EDBM only,
             never submitted to public tender -- the conditional branch
             of the flagship check)"
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Antananarivo Fabril SARL" :portal "Loi n°2016-055 portant Code des Marchés Publics filing (e-GP transactional portal at app.armp.mg)"
             :base-fee 340000 :monthly-rate 19000 :monitoring-months 12
             :claimed-fee 568000.0
             :requires-armp-clearance? true :on-armp-ineligibility-list? false
             :armp-recidivism-finding? false :armp-prior-exclusion-duration-years nil
             :drafted? false :submitted? false
             :jurisdiction "MDG" :status :intake}
    "eng-2" {:id "eng-2" :operator "Toamasina Logistics SA" :portal "Loi n°2016-055 portant Code des Marchés Publics filing (e-GP transactional portal at app.armp.mg)"
             :base-fee 340000 :monthly-rate 19000 :monitoring-months 12
             :claimed-fee 568000.0
             :requires-armp-clearance? true :on-armp-ineligibility-list? true
             :armp-recidivism-finding? false :armp-prior-exclusion-duration-years 2
             :drafted? false :submitted? false
             :jurisdiction "MDG" :status :intake}
    "eng-3" {:id "eng-3" :operator "Fianarantsoa Civil Works" :portal "Loi n°2016-055 portant Code des Marchés Publics filing (e-GP transactional portal at app.armp.mg)"
             :base-fee 340000 :monthly-rate 19000 :monitoring-months 12
             :claimed-fee 568000.0
             :requires-armp-clearance? true :on-armp-ineligibility-list? false
             ...
             :drafted? false :submitted? false
             :jurisdiction "MDG" :status :intake}
    "eng-4" {:id "eng-4" :operator "Mahajanga Civic Supply" :portal "Loi n°2016-055 portant Code des Marchés Publics filing (e-GP transactional portal at app.armp.mg)"
             :base-fee 340000 :monthly-rate 19000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "MDG" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nosy Be Regional Traders" :portal "EDBM Guichet Unique investment registration (Loi n°2007-036) -- no ARMP public-procurement filing"
             :base-fee 160000 :monthly-rate 11000 :monitoring-months 4
             :claimed-fee 204000.0
             :requires-armp-clearance? false :on-armp-ineligibility-list? false
             :armp-recidivism-finding? false :armp-prior-exclusion-duration-years nil
             :drafted? false :submitted? false
             :jurisdiction "MDG" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-armp-clearance? on-armp-ineligibility-list?
                               armp-recidivism-finding? armp-prior-exclusion-duration-years
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-armp-clearance?)      (assoc :engagement/requires-armp-clearance? requires-armp-clearance?)
    (some? on-armp-ineligibility-list?)   (assoc :engagement/on-armp-ineligibility-list? on-armp-ineligibility-list?)
    (some? armp-recidivism-finding?)      (assoc :engagement/armp-recidivism-finding? armp-recidivism-finding?)
    (some? armp-prior-exclusion-duration-years)
    (assoc :engagement/armp-prior-exclusion-duration-years armp-prior-exclusion-duration-years)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-armp-clearance? :engagement/on-armp-ineligibility-list?
   :engagement/armp-recidivism-finding? :engagement/armp-prior-exclusion-duration-years
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-armp-clearance? (boolean (:engagement/requires-armp-clearance? m))
     :on-armp-ineligibility-list? (boolean (:engagement/on-armp-ineligibility-list? m))
     :armp-recidivism-finding? (boolean (:engagement/armp-recidivism-finding? m))
     :armp-prior-exclusion-duration-years (:engagement/armp-prior-exclusion-duration-years m)
